...
_throttle_last_mono: dict[str, float] = {}
...
def msg_out_throttled(text: str, *, key: str, interval_s: float = 5.0) -> None:
    ...
    now = time.monotonic()
    with _throttle_lock:
        last = _throttle_last_mono.get(key, 0.0)
        if now - last < interval_s:
            return
        _throttle_last_mono[key] = now
    msg_out(text)


# 限流通道：key 与间隔（秒）。新增 msg_out_throttled 时请在此登记，避免魔法字符串分散。
THROTTLE_KEY_WINDOW_SIZE = "window.size_mismatch"
THROTTLE_INTERVAL_WINDOW_SIZE_S = 3.0

THROTTLE_KEY_NON_WINDOWS_HWND = "platform.non_windows_hwnd"
THROTTLE_INTERVAL_NON_WINDOWS_HWND_S = 5.0

THROTTLE_KEY_ADMIN_NOT_ELEVATED = "admin.not_elevated"
THROTTLE_INTERVAL_ADMIN_WARN_S = 3.0

THROTTLE_KEY_WGC_START_FAILED = "wgc.start_failed"
THROTTLE_INTERVAL_WGC_START_FAILED_S = 5.0
...
def maybe_warn_window_size(cw: int, ch: int) -> None:
    ...
    if abs(cw - REQUIRED_CLIENT_WIDTH) <= _WINDOW_SIZE_TOLERANCE_PX and abs(ch - REQUIRED_CLIENT_HEIGHT) <= _WINDOW_SIZE_TOLERANCE_PX:
        return
    msg_out_throttled(
        f"【提示】游戏窗口客户区当前为 {cw}×{ch}，请设为 {REQUIRED_CLIENT_WIDTH}×{REQUIRED_CLIENT_HEIGHT}，"
        "否则模板匹配与自动化可能异常。",
        key=THROTTLE_KEY_WINDOW_SIZE,
        interval_s=THROTTLE_INTERVAL_WINDOW_SIZE_S,
    )
...
def maybe_warn_non_windows_game_hwnd() -> None:
    ...
    msg_out_throttled(
        _NON_WINDOWS_GAME_HWND_WARN_TEXT,
        key=THROTTLE_KEY_NON_WINDOWS_HWND,
        interval_s=THROTTLE_INTERVAL_NON_WINDOWS_HWND_S,
    )
...
def _admin_warn_worker() -> None:
    interval = THROTTLE_INTERVAL_ADMIN_WARN_S
    while True:
        ...
        if not runs_as_elevated():
            msg_out_throttled(
                _ADMIN_WARN_TEXT,
                key=THROTTLE_KEY_ADMIN_NOT_ELEVATED,
                interval_s=interval,
            )
        if _admin_warn_stop.wait(interval):
            return


def start_admin_warn_loop() -> None:
    """非管理员时按 THROTTLE_INTERVAL_ADMIN_WARN_S 往 msg 日志追加提示（与现有终端轮询共用）。"""
